...
To investigate a factor's effect on a variable, we used Analysis of variance (ANOVA), a collection of statistical models and their associated estimation procedures used to analyze the differences among means. For following this procedure, we started exploration by making boxplots for the levels of each factor (Figure 1). It is obvious the mean of the leak for species 1 (s1) and 2 (s2) are different, showing a difference in a mean leak by factor variety. This also holds true for factors regime and temperature.  

```{r echo=FALSE, fig.cap=" Interaction plot of leak for each level of variety and temperature, with different regimes represented by color and symbol.", fig.align="center"}
...
In the next stage, we investigated the effect of each factor depending on the other factors; The profile plot of sample means (i.e., the interaction plot) shows a clear interaction between variety and regime; the difference in the mean lengths for regime R and regime C depends on the varieties, with larger variety differences for regime R than for regime C. This indicates in addition to the main effects. It is better to include interaction effects in the model to analyze the data and inference if the main effects and interaction effects are statistically significant.

The response variable, $Leak_{ijkr}$, is modeled as follows:


$$Leak_{ijkr} = \mu + \alpha_i + \beta_{ij} + \gamma_k + ( \alpha \times \beta){ij} + (\alpha \times \gamma){ik} + (\beta \times \gamma){jk} + (\alpha \times \beta \times \gamma){ijk} + \epsilon_{ijkr}$$
